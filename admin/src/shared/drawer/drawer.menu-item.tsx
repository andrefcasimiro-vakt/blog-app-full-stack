...
const DrawerMenuItem = ({
  ...
  return (
    <React.Fragment>
      <ListItemLink open={open} onClick={handleClick} renderArrow={!drawerMinimized}>
        <div className={classes.buttonContent}>
          <Icon />
          <Typography className={classes.displayName} variant="h6" color="textPrimary">
            {!drawerMinimized && drawerMenuItem.displayName}
          </Typography>
        </div>
      </ListItemLink>
      <Collapse component="li" in={open} timeout="auto" unmountOnExit>
        {pathOr([], ['children'], drawerMenuItem).map((Component: any, index) => (
