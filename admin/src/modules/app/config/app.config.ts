...
export const config = {
  app: {
    ...
    options: {
      ...
      logUserOutOnAuthError: extractEnv('APP_OPTIONS_LOG_USER_OUT_ON_AUTH_ERROR') || true,
    }
  },
  http: {
    customHeaders: {
      accessToken: extractEnv('HTTP_CUSTOM_HEADERS_ACCESS_TOKEN') || 'x-access-token',
      refreshToken: extractEnv('HTTP_CUSTOM_HEADERS_REFRESH_TOKEN') || 'x-refresh-token',
    },
  },
  graphql: {
